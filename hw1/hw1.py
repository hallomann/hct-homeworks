...
# УСКОРИТЬ kNN через np !!!
# время решения на kNN, NCD и через speedup
...
def parallel_processing(n_jobs, backend):
    ...
    - backend: используемая серверная часть для распараллеливания ("loki" для процессов, "threading" для потоков).
    """
    start_time = time.time()
    predicts = Parallel(
...
num_jobs_list = [1, 2, 3, 4, 5, 6, 7, 8]
backends = ["loky", "threading"]
times = {}  # словарь для хранения времени обработки для каждого сервера

for backend in backends:
    times[backend] = []  # список для сохранения времени для текущего backend
    for n_jobs in num_jobs_list:
        # выполняем параллельный процессинг
        predicts_parallel, parallel_time = parallel_processing(n_jobs, backend)
        times[backend].append(parallel_time)

# вычисляем точность последовательных прогнозов
accuracy = accuracy_score(Y_test, predicts_serial)  # сравним предсказания с истинными значениями
print("Accuracy: {:.2f}%".format(accuracy * 100))

speedup = serial_time / parallel_time
print(f"Speedup is: {speedup}")

# график производительности: время обработки в зависимости от number of jobs
for backend in backends:
    plt.plot(num_jobs_list, times[backend], label=f"{backend} backend")  # время построения для каждого backend

plt.title("Processing Time depends of Number of Jobs")
