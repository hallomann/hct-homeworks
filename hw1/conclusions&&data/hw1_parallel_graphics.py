...
def optimized_parallel_knn(X_train, Y_train, X_test, C_train, k=101, n_jobs=-1, backend='loky'):
    ...
    predicts = Parallel(n_jobs=n_jobs, backend=backend)(delayed(predict)(x1) for x1 in X_test)
    return predicts
    ...
    for backend in backends:
        ...
        for n_jobs in cores_range:
            ...
            def predict(x1):
                ...
                top_k_class = Y_train_np[sorted_idx]
                return Counter(top_k_class).most_common(1)[0][0]
            _ = Parallel(n_jobs=n_jobs, backend=backend)(delayed(predict)(x1) for x1 in X_test)
...
def main():
    ...
    print(f"The pre-calculation is completed in {time.time() - start_time:.2f} seconds.\n")
    ...
    print(f"The optimized serial implementation took {optimized_serial_time:.2f} seconds.")
    ...
    results, cores_range = optimized_parallel_knn_variants(X_train, Y_train, X_test, C_train, k, backends=backends)
    ...
    plt.axhline(y=original_time, color='r', linestyle='--', label='The original implementation')
    plt.xlabel('Number of cores')
    plt.ylabel('Execution time (seconds)')
    plt.title('Comparing the performance of different backends and the number of cores')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    speedup_serial = original_time / optimized_serial_time
    plt.axhline(y=speedup_serial, color='g', linestyle='--', label='Accelerating consistent implementation')
    plt.xlabel('Number of cores')
    plt.ylabel('x-factor')
    ...
    for backend in backends:
        for n, t in zip(cores_range, results[backend]):
            print(f"Backend: {backend}, Cores: {n}, Time: {t:.2f} seconds, Acceleration: {original_time / t:.2f}x")
